...
public class Car {
    ...
    // 기능
    // 생성자 : new 연산자를 통해서 객체가 생성될 때 생성된 객체의 필드값 초기화 + 기능 수행 역할

    public Car() {
        System.out.println("기본 생성자로 Car 객체 생성");
    }

    // 매개변수 : 생성자나 메서트 호출 시 ()안에 작성되어 전달되는 값을 저장하는 변수
    ...
    // 오버로딩 -> 같은 클래스 내에 동일한 이름의 메서드(생성자 포함)를 여러개 작성하는 기법

    // -> 하나의 이름으로 여러 기능을 수행할 수 있게 하는 것

    // 오버로딩 조건 -> 메서드의 이름이 동일, 매개변수의 개수, 타입, 순서 중 1개라도 달라야 함.
...
//        this.maxSpeed = maxSpeed;
    ...
    // 매개변수 명은 다르지만 개수, 타입, 순서가 동일하기 때문에 에러발생, 오버로딩 불가하다
